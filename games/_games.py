...
class Die:
    ...
    def roll(self) -> int:
        """Rolls the die and returns a number between 1-6."""
...
def simulate(num_players: int, num_rolls: int) -> List[Tuple[int, int]]:
    ...
    counter = Counter(results).items()  # dict_items([(3, 5514), (10, 8330)]
    sorted_counter = sorted(counter, key=lambda x: x[1], reverse=True)  # [(7, 16825), (6, 13908)]

    # test
    most_common = [x[0] for x in sorted_counter]  # [7, 6, 8, 5, 9, 4, 10, 3, 11, 2, 12]
    test_results(most_common)

    return sorted_counter


def test_results(most_common: List[int]) -> None:
    """Tests if simulation results are accurate."""
